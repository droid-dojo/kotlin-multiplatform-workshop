...
Die Koordinaten oben markieren die Mitte Deutschlands - setzen Sie stattdessen die Ihrer eigenen Stadt ein (zu finden über die [Geocoding-Suche von Open-Meteo](https://open-meteo.com/en/docs/geocoding-api) oder jede Kartenanwendung).
...
* Das **explizite Backing Field** (`field = …`, neu in Kotlin 2.4) ersetzt das klassische Pärchen aus privatem `MutableStateFlow` und öffentlichem `StateFlow`: Nach außen ist `uiState` ein unveränderliches `StateFlow`, innerhalb des ViewModels greift `uiState.value = …` direkt auf das `MutableStateFlow`-Field zu.
