...
> 📘 Die Theorie zu dieser Übung finden Sie im Handout: [Modul 5: Shared Logic in der Praxis - Coroutines, Flow & ViewModels](HANDOUT.md#modul-5-shared-logic-in-der-praxis---coroutines-flow--viewmodels) und [Modul 6: UI-Integration - Compose Multiplatform & SwiftUI](HANDOUT.md#modul-6-ui-integration---compose-multiplatform--swiftui).
...
Ziehen Sie eine dünne Schicht zwischen API und ViewModel ein: `class WeatherRepository(private val api: WeatherApi)` mit `suspend fun currentWeather(): CurrentWeather` (die Koordinaten Ihrer Stadt wandern hierhin). In Übung 2.2 wächst diese Klasse zur Offline-First-Schicht - die UI wird davon nichts merken.

### Schritt 3: Das ViewModel

`class WeatherViewModel(private val repository: WeatherRepository) : ViewModel()` - mit einem `StateFlow`, dessen `MutableStateFlow` als explizites Backing Field (`field = …`, neu in Kotlin 2.4) direkt an der Property lebt, und einer `refresh()`-Funktion, die in `viewModelScope.launch { ... }` lädt (Vorlage: [Handout, Modul 5.4: Gemeinsame ViewModels - Jetpack ViewModel überall](HANDOUT.md#54-gemeinsame-viewmodels-jetpack-viewmodel---überall)). Erster Refresh im `init`-Block.
