...
> 📘 Die Theorie zu dieser Übung finden Sie im Handout: [Modul 4: Multiplatform Networking mit Ktor](HANDOUT.md#modul-4-multiplatform-networking-mit-ktor). Die fertigen Dependency-Einträge stehen in [Anhang A: Setup & Dependencies für die Übungen, Schritt 1 + 2](HANDOUT.md#anhang-a-setup--dependencies-für-die-übungen).

---

## 🔍 Die Ausgangslage

Die App kennt seit Übung 1.1 ihre Plattform und ihre Zeitzone - aber Wetterdaten hat sie noch keine. Der Request, den wir bauen (probieren Sie ihn im Browser aus!) - die Koordinaten unten markieren die Mitte Deutschlands, setzen Sie die Ihrer eigenen Stadt ein (zu finden über die [Geocoding-Suche von Open-Meteo](https://open-meteo.com/en/docs/geocoding-api)):

```
https://api.open-meteo.com/v1/forecast?latitude=51.16&longitude=10.45&current=temperature_2m,weather_code,wind_speed_10m&timezone=auto
```
...
Retrofit und Gson helfen uns hier nicht - beide sind JVM-only ([Handout, Modul 4.1: Warum Ktor?](HANDOUT.md#41-warum-ktor)). Unsere Werkzeuge: **[Ktor](https://ktor.io)** und **[kotlinx.serialization](https://github.com/Kotlin/kotlinx.serialization)**.
...
* Die App zeigt auf Desktop, Android und im Web die aktuelle Temperatur Ihrer Stadt.

## 🛠 Die Aufgaben im Detail

### Schritt 1: Dependencies einbinden

Ergänzen Sie `gradle/libs.versions.toml` und `shared/build.gradle.kts` um das **Serialization-Plugin**, **Ktor** (Core, ContentNegotiation, kotlinx-json, Logging) und die **Engines pro Source Set** - die fertigen Einträge stehen im [HANDOUT.md, Anhang A: Setup & Dependencies für die Übungen](HANDOUT.md#anhang-a-setup--dependencies-für-die-übungen). Danach: Sync!
...
- [ ] Desktop, Android und Web zeigen die aktuelle Temperatur Ihrer Stadt.
